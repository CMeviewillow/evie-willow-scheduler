...
function scheduleSingleJob(job, state, holidays, settings, impact, opts = {}) {
  ...
  if (pinnedInstallDate) {
    // Honour target unless production can't finish in time.
...
function App() {
  ...
  useEffect(() => {
    if (!loading && !IS_READONLY) {
      window.storage.set("ew-jobs", JSON.stringify(jobs)).catch(console.error);
    }
  }, [jobs, loading]);

  useEffect(() => {
    if (!loading && !IS_READONLY) {
      window.storage.set("ew-settings", JSON.stringify(settings)).catch(console.error);
    }
  }, [settings, loading]);

  useEffect(() => {
    if (!loading && !IS_READONLY) {
      window.storage.set("ew-dismissed-reminders", JSON.stringify(dismissedReminders)).catch(console.error);
    }
  }, [dismissedReminders, loading]);

  useEffect(() => {
    if (!loading && !IS_READONLY) {
      window.storage.set("ew-dismissed-warnings", JSON.stringify(dismissedWarnings)).catch(console.error);
    }
  ...
  useEffect(() => {
    ...
    if (seriousActiveSig !== lastSeenSeriousSig) {
